    ...
    >
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="p-1.5 hover:bg-neutral-800 text-gray-300 hover:text-white rounded-[8px] cursor-pointer topbar-btn border border-transparent"
      ...
      <div className="hidden md:flex flex-1 max-w-lg mx-6 relative">
        <div className="w-full relative flex items-center">
          <input
            ...
            className="w-full h-9 pl-3 pr-16 border border-neutral-700 bg-neutral-800 text-white placeholder-neutral-400 text-xs focus:outline-none focus:border-neutral-500 focus:bg-white focus:text-neutral-900 rounded-[8px]"
          />
          <span className="absolute right-2 px-1.5 py-0.5 rounded-[8px] border border-neutral-700 text-[12px] bg-neutral-950 text-neutral-400 select-none">Ctrl + K</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 border border-neutral-700 rounded-[8px] text-[12px] font-semibold bg-neutral-800 text-white hover:bg-neutral-700 cursor-pointer topbar-btn"
          >
          ...
          {showRoleDropdown && (
            <div className="absolute right-0 mt-1 w-48 rounded-[8px] border border-neutral-800 bg-neutral-900 shadow-xl p-1 z-50 text-white">
              ...
              {Object.values(ROLES).map((role) => (
                <button
                  ...
                  className={`w-full text-left text-xs font-semibold p-2 rounded-[8px] transition-colors ${currentRole === role
        ...
        <div className="relative">
          <button
            ...
            className="relative p-1.5 hover:bg-neutral-800 text-gray-300 hover:text-white rounded-[8px] cursor-pointer topbar-btn"
            style={{ background: 'transparent', border: 'none', boxShadow: 'none' }}
          >
            <Bell className="h-4.5 w-4.5 text-gray-300" />
            {openTickets.length > 0 && (
              <span className="absolute -top-0.5 -right-0.5 h-2.5 w-2.5 rounded-[8px]-full bg-[#d13212] border border-neutral-900"></span>
            )}
          </button>

          {showNotifications && (
            <div className="absolute right-0 mt-2 w-80 rounded-[8px] border border-neutral-800 bg-neutral-900 shadow-xl p-3 z-50 text-white">
              <div className="flex items-center justify-between border-b border-neutral-800 pb-1.5 mb-2">
                ...
                <span className="text-[12px] border border-[#d13212] px-1 py-0.25 rounded-[8px] font-bold bg-[#38110b] text-[#ffd5cc] animate-pulse">LIVE</span>
              </div>
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {openTickets.length === 0 ? (
                  ...
                  openTickets.map((t) => (
                    <div key={t.id} className="p-2 border border-neutral-800 rounded-[8px] bg-neutral-950/50 text-xs">
        ...
        <div
          className="relative flex h-8 w-8 items-center justify-center border border-neutral-700 rounded-[8px] bg-neutral-800 text-white shadow-none overflow-hidden"
        >
          <span className="text-[12px] text-white font-bold z-10">AI</span>
          <span className="absolute bottom-0.5 right-0.5 h-2 w-2 rounded-[8px]-full bg-[#1d8102] border border-neutral-900"></span>
        </div>
      </div>
    </header>
  );
