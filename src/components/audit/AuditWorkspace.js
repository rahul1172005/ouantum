        ...
        <div>
          ...
          <div className="mb-4">
            ...
            <select
              ...
              className="w-full h-9 px-3 rounded-[8px] bg-black/40 border border-white/10 text-white-text focus:outline-none focus:border-[#ea580c] transition-colors font-mono text-xs"
            >
          ...
          <div className="space-y-3 font-mono text-xs mb-4">
            {checklist.map((item) => (
              <label 
                key={item.id}
                className="flex items-start gap-2.5 p-2 rounded-[8px] bg-black/30 border border-white/5 hover:border-[#ea580c]/20 cursor-pointer transition-colors"
              >
                <input 
                  type="checkbox" 
                  checked={item.checked} 
                  onChange={() => handleToggleCheck(item.id)}
                  className="mt-0.5 rounded-[8px] border-white/15 bg-transparent text-[#ea580c] accent-[#ea580c] focus:ring-0 focus:outline-none"
                />
        ...
        <div className="space-y-3 border-t border-white/5 pt-4">
          ...
          <div className="flex gap-2">
            <button 
              ...
              className="flex-1 h-10 bg-[#ea580c]/15 hover:bg-[#ea580c]/25 text-[#ea580c] border border-[#ea580c]/35 rounded-[8px] font-bold font-mono text-xs uppercase flex items-center justify-center gap-1.5 transition-colors"
            >
      ...
      <div className="lg:col-span-2 glass-panel rounded-[8px]-xl border border-[#ea580c]/15 bg-secondary-bg/60 p-4 flex flex-col justify-between min-h-[460px]">
        <div>
          <div className="flex items-center justify-between border-b border-white/5 pb-2 mb-3">
            <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-[#ea580c] flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex gap-2">
              <button 
                ...
                className={`w-5 h-5 rounded-[8px]-full border bg-[#ff4d6d] ${brushColor === '#ff4d6d' ? 'ring-2 ring-[#ea580c]' : ''}`}
              />
              <button 
                onClick={() => setBrushColor('#ff9f43')} // Warning defect color
                className={`w-5 h-5 rounded-[8px]-full border bg-[#ff9f43] ${brushColor === '#ff9f43' ? 'ring-2 ring-[#ea580c]' : ''}`}
              />
              <button 
                onClick={() => setBrushColor('#00ffb2')} // Normal inspection indicator
                className={`w-5 h-5 rounded-[8px]-full border bg-[#00ffb2] ${brushColor === '#00ffb2' ? 'ring-2 ring-[#ea580c]' : ''}`}
              />
            </div>
          </div>
