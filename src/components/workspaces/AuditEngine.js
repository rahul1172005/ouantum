...
export default function AuditEngine({ selectedElement, setSelectedElement }) {
  ...
  const handleVoiceRecord = () => {
    if (isRecording) {
      ...
      // Auto-append parsed voice info as a checklist item or evidence log
      const newEvidence = {
        id: Date.now(),
  ...
  const handleFileUpload = (e) => {
    e.preventDefault();
    const mockFiles = [
      'spall_thermal_scan.png',
      'core_tensile_report.pdf',
      'deflection_matrix_log.csv'
    ];
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 font-mono text-xs text-black">
      ...
      <div className="xl:col-span-2 space-y-6">
        ...
        <div className="border-2 border-black p-4 bg-white space-y-4">
          <div className="flex items-center justify-between border-b border-black pb-2">
            <span className="font-bold uppercase tracking-wider text-[11px] flex items-center gap-1.5">
              <ClipboardCheck className="h-4.5 w-4.5" /> SMART INSPECTION TASK LISTS
            </span>
            <div className="flex border border-black rounded overflow-hidden">
              {['Structural', 'Safety', 'Material'].map((cat) => (
                <button
                  ...
                  className={`px-3 py-1 border-r last:border-r-0 border-black transition-colors ${
            ...
            {auditChecklist.filter(item => item.category === activeCategory).map((item) => (
              ...
              >
                <div className="mt-0.5">
                  {item.checked ? (
                    <CheckSquare className="h-4.5 w-4.5 text-black" />
                  ) : (
                    <Square className="h-4.5 w-4.5 text-black" />
                  )}
                </div>
                <div>
        ...
        <div className="border-2 border-black p-4 bg-white space-y-3">
          <p className="font-bold border-b border-black pb-1.5 uppercase text-[10px]">AI voice Inspection assistant</p>
      ...
      <div className="xl:col-span-1 space-y-6">
        ...
        {/* Upload portal */}
        <div className="border-2 border-black p-4 bg-white space-y-4">
          <p className="font-bold border-b border-black pb-1.5 uppercase text-[10px]">Evidence Upload Matrix</p>
        ...
        {/* Evidence files list */}
        <div className="border-2 border-black p-4 bg-white space-y-3">
          <p className="font-bold border-b border-black pb-1.5 uppercase text-[10px]">Audit Evidence Logs</p>
