...
const STATUS_STYLE = {
  ...
  FAIL:    'bg-black text-white border-black',
  PENDING: 'bg-white text-gray-500 border-[#d4d4d4] animate-pulse',
};

const NCR_STATUS = {
  OPEN:        'bg-black text-white border-black animate-pulse',
      ...
      <div className="zoho-card">
        ...
        <div className="zoho-card-body space-y-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 flex-wrap">
                {categories.map(cat => (
                  <button key={cat} onClick={() => setActiveCategory(cat)}
                    className={`text-[9px] px-2 py-1 border font-bold uppercase rounded-sm ${activeCategory === cat ? 'bg-black text-white border-black' : 'bg-white border-[#d4d4d4]'}`}>
                    {cat}
                  </button>
                ))}
              </div>
              <table className="w-full text-[10px]">
                ...
                <tbody>
                  {filtered.map(c => (
                    <tr key={c.id}>
                      <td><span className="text-[8.5px] font-bold text-gray-500 uppercase">{c.category}</span></td>
                      <td className="font-semibold">{c.item}</td>
                      <td className="text-gray-500">{c.method}</td>
                      <td>
                        <button onClick={() => cycleStatus(c.id)}
                          className={`text-[7.5px] px-1.5 py-0.5 border rounded-sm font-black uppercase cursor-pointer ${STATUS_STYLE[c.status]}`}>
                          {c.status}
                        </button>
                      </td>
                    </tr>
              ...
              <p className="text-[8px] text-gray-400 italic">Click status badge to cycle PASS → FAIL → PENDING</p>
            </div>
          )}
          ...
          {activeTab === 'NCR Log' && (
            <div className="space-y-3">
              {NCR_LOG.map(ncr => (
                <div key={ncr.id} className="p-3 border border-[#d4d4d4] bg-white rounded-sm cursor-pointer hover:bg-gray-50"
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <span className="font-black text-[10px]">{ncr.id}</span>
                      <span className="text-[8.5px] text-gray-500 ml-2">Raised: {ncr.raised}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={`text-[7px] px-1.5 py-0.5 border rounded-sm font-black uppercase ${ncr.severity === 'MAJOR' ? 'border-black text-black' : 'border-gray-400 text-gray-500'}`}>{ncr.severity}</span>
                      <span className={`text-[7px] px-1.5 py-0.5 border rounded-sm font-black uppercase ${NCR_STATUS[ncr.status]}`}>{ncr.status}</span>
                    </div>
                  </div>
                  <p className="text-[9.5px] font-bold text-gray-800 uppercase">{ncr.element}</p>
                  <p className="text-[9px] text-gray-600">{ncr.defect}</p>
                  <div className="flex justify-between mt-2 pt-1.5 border-t border-dashed border-gray-200">
                    <span className="text-[8px] text-gray-500"><User className="h-2.5 w-2.5 inline mr-0.5" />{ncr.assignedTo}</span>
                    <span className="text-[8px] text-gray-500"><Clock className="h-2.5 w-2.5 inline mr-0.5" />Due: {ncr.dueDate}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          {activeTab === 'Corrective Actions' && (
            <div className="space-y-3">
              {NCR_LOG.map(ncr => (
                <div key={ncr.id} className="p-3 border border-[#d4d4d4] bg-gray-50 rounded-sm">
                  <div className="flex justify-between items-center mb-1.5">
                    <span className="font-black text-[9.5px] uppercase">{ncr.id}</span>
                    <span className={`text-[7px] px-1.5 py-0.5 border rounded-sm font-black uppercase ${NCR_STATUS[ncr.status]}`}>{ncr.status}</span>
                  </div>
                  <p className="text-[9px] text-gray-800 font-bold">{ncr.element} — {ncr.defect}</p>
                  <div className="mt-2 p-2 border border-[#d4d4d4] bg-white rounded-sm">
                    <p className="text-[8px] text-gray-500 uppercase font-bold mb-0.5">Corrective Action</p>
                    <p className="text-[9px] text-gray-800">{ncr.action}</p>
                  </div>
                  <div className="flex gap-2 mt-2">
                    <button className="btn-skeuo text-[8px] uppercase">Verify Closure</button>
                    <button className="btn-skeuo text-[8px] uppercase">Reassign</button>
                    <button className="btn-skeuo-dark text-[8px] uppercase">Generate Report</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="zoho-card-footer">
          <span className="text-gray-500 font-semibold text-[9px] uppercase">Digital Site Inspection — Replace paper checklists with AI-assisted NCR workflow</span>
        </div>
      </div>
    </div>
  );
